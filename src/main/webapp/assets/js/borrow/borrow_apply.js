...
function queryFormSubmit(){
	...
	clearPage();
	$.ajax({
		url: basePath + "medical_record_borrow/ajax_query_medical_record",
		dataType: "json",
		data:submitData,
...
function addRows(medicalRecords){
	for(var index in medicalRecords){
		...
		if(borrowApplying && borrowApplying==1){
			...
			add_content.find("a").removeAttr("onclick").html("病案预览").attr("title","病案预览").click(function(){showBorrow(this)});
		}else{
			add_content.find("span[class='borrow_status']").html("可借阅");
			add_content.find("a").removeAttr("onclick").html("申请借阅").attr("title","申请借阅").click(function(){addBorrowSubmit(this)});
...
function addBorrowSubmit(obj){
	...
	layer.load(1);
	$.ajax({
		url: basePath + "medical_record_borrow/ajax_add_borrow_apply",
		type: "POST",
		dataType: "json",
		...
		success: function( data ) {
			...
			if(success){
				...
				if("1" == stateCode){
					layer.msg("申请借阅成功！");
					content.find("span[class='borrow_status']").html("借阅申请中");
					content.find("a").parent().html("借阅申请中");
				}else{
					layer.msg("申请借阅失败！");
				}
			}else{
...
function showBorrow(obj){
	...
	var visitGuid = content.attr("id");
	var basePath = $("#basePath").val();
	window.open(basePath+'medical_record_borrow/show_pdf_to_borrow?visitGuid='+visitGuid);
}
