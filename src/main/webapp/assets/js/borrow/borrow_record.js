...
function queryFormSubmit(){
	...
	clearPage();
	$.ajax({
		url: basePath + "medical_record_borrow/ajax_query_borrow_record",
		dataType: "json",
		data:submitData,
...
function addRows(medicalRecords){
	for(var index in medicalRecords){
		...
		add_content.find("span[class='borrow_status']").html(medicalRecord.reply_status_name);
		add_content.find("span[class='validity_date_format']").html(medicalRecord.validity_date_format);
		
		var showSatus = medicalRecord.show_status;
		var borrowPermit = medicalRecord.permit;
		if(showSatus && showSatus==1){
			add_content.find("a").html("病案预览");
			
		}else{
			add_content.find("a").parent().html("");
		}
