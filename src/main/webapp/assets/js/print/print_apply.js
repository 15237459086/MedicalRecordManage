...
function queryFormSubmit(){
	...
	clearPage();
	$.ajax({
		url: basePath + "medical_record_print/ajax_query_print",
		dataType: "json",
		data:submitData,
...
function addApplyFormSubmit(){
	
	var basePath = $("#basePath").val();
	var submitData = $('#applyForm').serialize();
	
	layer.load(1);
	$.ajax({
		url: basePath + "medical_record_print/ajax_add_print_apply",
		type: "POST",
		dataType: "json",
		data:submitData,
		success: function( data ) {
			...
			if(success){
				...
				if("1" == stateCode){
					layer.msg("打印申请成功！");
					layer.close(addApplyFormIndex);
					var printerApplyItem = data['data'];
					parent.document.getElementById("mainContent").src=basePath+"medical_record_print/print_operate?printerApplyItemId="+printerApplyItem.id;
				}else{
					layer.msg("打印申请失败！");
				}
			}else{
...
function applyFormShow(obj){
	
	var add_content=$("#applyLayerDiv").clone();
	add_content.find("form").attr("id","applyForm");
	addApplyFormIndex = layer.open({
		  type: 1,
		  title:'打印申请',
		  ...
		  success: function(layero, index){
			  ...
			  $("#applyForm input[name='visitGuid']").val(content.attr("id"));
			  $("#applyForm input[name='onlyId']").val(content.find(".only_id").html());
			  $("#applyForm input[name='patientName']").val(content.find(".patient_name").html());
			  $("#applyForm input[name='idNumber']").val(content.find(".id_number").html());
			  $("#applyForm input[name='mrId']").val(content.find(".mr_id").html());
			  $("#applyForm input[name='visitNumber']").val(content.find(".visit_number").html());
			  $("#applyForm input[name='outHospitalDateTime").val(content.find(".out_hospital_date").html());
			  
			  $("#applyForm select[name='claimerRelativeRelationCode'],[name='applyPrinterTypeCode']").each(function(){
